...
Two facts about the SigNoz side set the terms. The body of a SigNoz object is a structured document with its own internal shape: some resources use a `kind`/`spec` model, some a `schemaVersion`/`spec` model, and some both. And that shape moves on SigNoz's release cadence: an untyped resource becomes a typed one, and a payload schema that has already been replaced once can be replaced again.
...
It is genuinely tempting — the flattest YAML, no extra indentation — and it is what generated APIs tend to produce, since a spec derived from a vendor's API model naturally lands at the spec root. What decides it is what happens next. With the payload owning the root, the operator's own controls have nowhere to go: they migrate into annotations, which [core-spec.md](core-spec.md) rules out for anything our controller consumes and which a deletion policy is commonly carried as ([ACK — deletion policy](https://aws-controllers-k8s.github.io/community/docs/user-docs/deletion-policy/)), or the first control that must live in the spec arrives as a breaking change and a new API version, because the fence that should have been there from the start has to be introduced around an already-published payload. Our equivalent field, `providerConfigRef`, is required on day one, so a flat spec would start us where that migration ends.
...
Invert the nesting: group the shared controls under one key and let each kind's own fields own the spec root. Operators do ship this, keeping the remote object's fields flat in the spec and fencing the operator's own behaviour behind a single key documented as interpreted locally and never sent upstream ([ASO — resources API](https://azure.github.io/azure-service-operator/reference/resources/v1api20200601/), [KEDA — ScaledObject specification](https://keda.sh/docs/latest/reference/scaledobject-spec/)).
...
**Rejected.** A string map sits outside the structural schema, so every field inside it is a string the API server does not validate, default, or publish. The cost is visible in the manifests before it is visible anywhere else: counts and booleans have to be quoted to survive a `map[string]string`, so a threshold is written `'5'` and a flag `'false'` ([KEDA — Apache Kafka scaler](https://keda.sh/docs/latest/scalers/apache-kafka/)). The deeper cost is that validation, defaulting, enums, optionality and deprecation do not disappear when they leave the schema — they are rebuilt inside the controller, as a private tag language and a validator that reimplement what kubebuilder markers would have had the API server enforce at apply time.
...
The mirror is exact except where a body holds a discriminated union — an OpenAPI `oneOf` whose branch is chosen by the value of a sibling field, which recurs across SigNoz bodies wherever one field decides how another decodes. A structural schema cannot express it, because it *"does not set `description`, `type`, `default`, `additionalProperties`, `nullable` within an `allOf`, `anyOf`, `oneOf` or `not`"* ([Custom resource definitions](https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#specifying-a-structural-schema)): a branch may constrain a value but may not give a field its type, so no discriminator can select one. The typed form therefore carries every branch's fields in one struct at that position, with CEL enforcing the subset each discriminator value admits, and the adapter assembles the body. The wire shape is unchanged. Kinds whose bodies hold no such union mirror it exactly.
...
The kind supplies those two methods; the adapter beside it supplies what needs to talk HTTP — the `List` or `Get` that [idempotency.md](idempotency.md) resolves identity with, and the status-code classification table [core-status.md](core-status.md) requires.
...
- Printer columns must come from status. No path under `objectTemplate` resolves on a `jsonSpec` object, so anything `kubectl get` shows comes from `CoreStatus`, or from a field the operator lifts out of the body it rendered — desired state it already holds locally, on either form.
...
- Adding a kind is declaring a `<Kind>Config` struct, its `objectTemplate` wrapper, `RenderBody` and `Identity` on the kind, and an adapter supplying the lookup and the classification table. Shared reconcile code does not change.
...
- [Kubernetes — PodTemplate API](https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-template-v1/)
...
- [Kubernetes — Custom resource definitions (structural schemas)](https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#specifying-a-structural-schema)
