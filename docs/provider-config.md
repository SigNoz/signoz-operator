...
SigNoz authenticates an API request with an HTTP header carrying a credential, and that credential holds the permissions of the service account that issued it. So the selection mechanism is not just plumbing: it determines whose credential a given namespace's resources write with, which is a security boundary, not a convenience. The header is not fixed: a deployment sends the key under `SIGNOZ-API-KEY` by default, but a reverse proxy or gateway in front of SigNoz may expect a different header name (`x-api-key`, a tenant header) or an `Authorization: Bearer <token>` scheme. Which header authenticates a request, and whether its value carries a scheme prefix, is a property of how SigNoz was deployed, not something the operator can assume.

## Constraints

- **The SigNoz API credential is org-scoped.** An API key grants its service account's permissions across the org; the API offers no way to narrow a single key to one dashboard or one team. This is a property of the remote API, not something the operator can tighten.
...
- **Operators disagree on where the endpoint and credential should live.** A quickstart wants them inline; a GitOps or Vault-backed install wants nothing in the resource at all — ideally the whole connection in one externally-managed Secret. The object must serve both rather than force one.
...
**Rejected.** One org-scoped credential means any namespace that can create a resource writes with the cluster's single most-privileged identity. A team that should only manage its own dashboards can, through the operator, act as the whole org. Kubernetes RBAC on the resource and SigNoz RBAC on the credential cannot be made to agree, because there is only one credential for every tenant. The convenience of no extra object is real but secondary; the credential-scoping failure is the disqualifier.
...
**Rejected.** A selector matches a credential by label rather than naming it, so which exact Secret — and whose RBAC governs it — turns on labels that can drift or collide, and the operator resolves a credential by searching rather than by name. A credential is named, not searched for: the direct `authenticationRef` KEDA uses for a `TriggerAuthentication` gives one unambiguous owner and one Secret whose RBAC is the whole story.
...
Keep the connection entirely in a Kubernetes `Secret` selected by a well-known label, as Argo CD does for cluster and repository credentials — no custom resource at all.

**Accepted, in part.** Its core instinct is right and we keep it: credential material belongs in a `Secret`, so that native Secret RBAC and encryption-at-rest apply and the credential never sits in plaintext inside a spec. What a bare labeled Secret cannot give is a typed, validated endpoint, a place for TLS settings, a health/`Ready` status, or `kubectl` discoverability — all of which a backend definition wants. So we do not reject the Secret; we reject making it the *whole* object. The accepted design is a thin CRD that holds the non-secret configuration and *references* a Secret for the credential — the credential stays where Argo CD keeps it, and the typed surface lives in the CRD.
...
### The kind is named `ProviderConfig`, not `Provider` or `Connection`

The object plays the role Crossplane calls a [`ProviderConfig`](https://docs.crossplane.io/latest/packages/providers/): the endpoint-and-credentials configuration that managed resources reference to reach an external API. The name is deliberate on two counts. First, a bare `Provider` means something else across the ecosystem — in Crossplane and Cluster API a *Provider* is an installed controller/package that adds new APIs, and in External Secrets and Velero `provider` is an inline field naming a backend type — so a `Provider` kind here would read as "an installed plugin," which this is not. Second, `Connection` reads as a live, stateful thing (a pooled DB connection) when this is a declarative pointer. The peer projects that model exactly this role reach for a config-flavoured name for the same reason: cert-manager's [`Issuer`](https://cert-manager.io/docs/concepts/issuer/) and External Secrets' [`SecretStore`](https://external-secrets.io/latest/api/secretstore/). We follow the `providerConfigRef` reference convention that goes with the name.
...
Reusing `corev1.SecretKeySelector` gives `{name, key}` that is name-only — same-namespace by construction, the property the constraints demand — and its deepcopy is already generated. `fieldRef` and `resourceFieldRef` are pod- and container-only and are left out. Kubernetes does not itself enforce `value` exclusive-or `valueFrom` for env vars (the kubelet just lets `value` win); the `HeaderAuth` CEL rule above does, so a mistake is rejected rather than silently resolved.
...
status:
  conditions: [{type: Ready, ...}]        # from a health probe against the endpoint
```
...
A cluster-scoped object has no namespace of its own, so its name-only Secret reference resolves in the operator's namespace — exactly as a KEDA `ClusterTriggerAuthentication` reads its Secrets from KEDA's own namespace rather than the workload's. The shared credential is therefore one a cluster administrator places beside the operator, and a namespaced resource that names a `ClusterProviderConfig` uses that credential without holding a copy of it.
...
- [Crossplane — Providers and ProviderConfig](https://docs.crossplane.io/latest/packages/providers/)
