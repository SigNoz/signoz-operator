...
primus (`signoz/primus`, private) provides the repo's `skaff` codegen, the Go checks (`go-fmt`/`go-lint`/`go-deps`/`go-test`) and `controllergen`. This skill installs it and sets `PRIMUS_HOME`. Requires `gh` with SigNoz org access.
