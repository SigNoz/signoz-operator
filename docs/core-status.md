...
| Type | Meaning |
|---|---|
| `Terminal` | The desired state is wrong and no retry will help — a validation rejection, a name already taken, a body that will not parse. A stable state: the reconciler settles here and does not requeue, on `interval` or otherwise. |
| `Recoverable` | A failure that a change to this resource would not fix — 429, 5xx, a dial or connection error, a credential the server rejects. The reconciler requeues at [`retryInterval`](core-spec.md) (falling back to `interval`). |
...
Which HTTP status maps to `Terminal` versus `Recoverable` is fixed, the same for every kind: 400 and 409 are `Terminal` — a rejected body and a collision are failures no retry will fix — and every other non-2xx status is `Recoverable`, so a status the classifier does not recognise keeps retrying. 401 and 403 are among them: what fixes a rejected credential is a rotated Secret, or a permission granted to the identity the credential belongs to, and neither is an edit to the resource that was refused. Granting that permission in SigNoz moves nothing in the cluster for a watch to see, so a resource that settled on `Terminal` would stay there after the cause was fixed; retrying is what lets it recover on its own. Method-specific meaning stays in the reconcile flow, not in the classifier: a 409 on create is intercepted and resolved by lookup before it can settle ([idempotency.md](idempotency.md)), and a failed delete is retried whatever its classification, so reclaim never strands on a settled state.
