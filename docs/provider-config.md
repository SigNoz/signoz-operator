...
A rotation therefore lands: the Secret is updated, the provider-config controller resolves again and records the new `resourceVersion`, and every resource naming that config is requeued and sends its next request with the new credential. This is also what makes a rejected credential recoverable. A credential the server rejects is `Terminal` and is not requeued on `interval` ([core-status.md](core-status.md)); the requeue that follows a credential change is what brings those resources back without a spec edit.
